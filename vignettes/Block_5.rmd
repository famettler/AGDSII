...
ctrl_cluster <- caret::trainControl(
  method = "cv",
  index = folds,
)

model_spatial_cv <- caret::train(
  pp,
  data = dfs |> tidyr::drop_na(),
  method = "ranger",
  trControl = ctrl_cluster,
  tuneGrid = expand.grid(
    .mtry = 3,
    .min.node.size = 12,
    .splitrule = "variance"
...
knitr::kable(model_spatial_cv$resample)
cat(
  "Mean RMSE:", mean(model_spatial_cv$resample$RMSE),
  "Mean R^2:", mean(model_spatial_cv$resample$Rsquared)
)
...
train_test_by_fold <- function(df_spat, idx_train, idx_val) {
  
  train_df <- df_spat |>
    dplyr::slice(idx_train) |>
    dplyr::select(elv, mat, map, ndep, mai, Species, leafN) |>
    tidyr::drop_na()
  ...
  train_df$leafN <- as.numeric(train_df$leafN)
  val_df$leafN   <- as.numeric(val_df$leafN)
  train_df$Species <- as.factor(train_df$Species)
  val_df$Species   <- as.factor(val_df$Species)
  
  x_train <- train_df |> dplyr::select(-leafN)
  y_train <- train_df$leafN
  
  x_val <- train_df |> dplyr::select(-leafN)
  y_val <- train_df$leafN
  
  mod <- ranger::ranger(
    x =  x_train, # data frame with columns corresponding to predictors
    y =  y_train, # a vector of the target values (not a data frame!)
    mtry = 3,
    min.node.size = 12,
    splitrule = "variance",
    seed = 101
  ...
  pred <- predict(mod, # the fitted model object
    data = x_val # a data frame with columns corresponding to predictors
  )$predictions

  rsq <- 1 - sum((y_val - pred)^2) / sum((y_val - mean(y_val))^2) # the R-squared determined on the validation set
  rmse <- sqrt(mean((pred - y_val)^2)) # the root mean square error on the validation set
...
group_folds_train <- purrr::map(
  seq(length(unique(df_spat$cluster))),
  ~ {
    df_spat |>
      select(cluster) |>
      mutate(idx = 1:n()) |>
...
train_test_by_fold <- function(idx_train, idx_val){
      data_pred <- df_spat |> dplyr::slice(idx_train)
      data_eval <- df_spat |> dplyr::slice(idx_val)
