...
As a further step of the data preaparation, I define the target (waterlog.100) and the predictor variables.
...
The output confirms, that we are dealing with a classification. We further see that Mtry is 9, which is the number of randomly picked predictors per split.
...
We see that in both data sets the case of unlogged soils is more frequent then watterlogged soil.
...
```{r}
set.seed(101)
mod_cv <- caret::train(df_train[, predictors_all],
  df_train[[target]],
  method = "ranger",
  trControl = caret::trainControl(method = "cv",
                        number = 5,
                        classProbs = F),
  tuneGrid = expand.grid(mtry = c(5, 9, 15),
                        splitrule = "gini",
